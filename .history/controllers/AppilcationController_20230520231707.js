...
const downloadDegreedocs = async (req, res) => {
  try {
    const {degree_no}=req.params.degree_reg_no;
console.log("obj",degree_no);
   console.log(req.params.degree_reg_no);
    // Get the subdirectory paths based on degree_reg_no
    const degreeImagesPath = path.join(__dirname, `../public/degreeImages/${req.params}`);
    const degreePdfsPath = path.join(__dirname, `../public/degreePdfs/${req.params}`);
